...
def extract_text_from_adf(adf):
    """从 Atlassian Document Format 提取文本"""
    texts = []
...
def generate_html(issues, status_counts, label_counts, assignee_counts):
    ...
    for issue in issues:
        ...
        <tr class="{sla_class}" 
            ...
            data-description="{issue['description'].lower()[:200]}"
            ...
            <td class="col-key-summary">
                ...
                <span class="issue-summary" title="{issue['summary']}">{issue['summary'][:60]}{'...' if len(issue['summary']) > 60 else ''}</span>
            </td>
            <td class="col-status">
            ...
            <td class="col-desc">
                <div class="description-cell">{issue['description'][:150]}{'...' if len(issue['description']) > 150 else ''}</div>
            </td>
