#!/usr/bin/env python3
"""
FY26_INIT 报告生成脚本 v5.0
从 SQLite 数据库读取数据并生成 JSON 和 HTML 报告
"""
...
def generate_report():
    ...
    initiatives_query = '''
        ...
        WHERE EXISTS (
            ...
            WHERE f.parent_key = i.key
            AND EXISTS (
                SELECT 1 FROM epics e
                WHERE e.parent_key = f.key
            )
        )
        ORDER BY i.key
    '''
    ...
    # 3. 孤儿 Feature（有 FY26_INIT 标签但无 Epic）
    orphan_features_query = '''
        SELECT f.key as feat_key, f.summary as feat_summary,
               f.status as feat_status, f.assignee as feat_assignee,
               f.parent_key as init_key
        FROM features f
        WHERE f.labels LIKE '%FY26_INIT%'
        AND NOT EXISTS (
            SELECT 1 FROM epics e WHERE e.parent_key = f.key
        )
        ORDER BY f.key
    ...
    stats = {
        ...
        'initiatives_with_epics': len(initiatives),
    ...
    print(f"\n📊 统计:")
    print(f"   - 总 Initiatives: {stats['total_initiatives']}")
    print(f"   - 有 Epic 的: {stats['initiatives_with_epics']}")
    print(f"   - 总 Epics (非 CNTIN): {stats['total_epics_all']}")
    print(f"   - 已关联的 Epics: {stats['total_epics_linked']}")
