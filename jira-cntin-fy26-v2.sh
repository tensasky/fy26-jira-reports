...
curl -s -u "${JIRA_USER_EMAIL}:${JIRA_API_TOKEN}" \
  ...
  }' > "$OUTPUT_DIR/cntin_fy26_init_${DATE_STR}.json"

INIT_COUNT=$(cat "$OUTPUT_DIR/cntin_fy26_init_${DATE_STR}.json" | python3 -c "import json,sys; d=json.load(sys.stdin); print(len(d.get('issues', [])))")
...
# 获取 Initiative keys
INIT_KEYS=$(cat "$OUTPUT_DIR/cntin_fy26_init_${DATE_STR}.json" | python3 -c "
import json,sys
...
echo "  Initiative keys: $(echo $INIT_KEYS | tr ',' ' ' | wc -w) 个"

# Step 2: 抓取关联的 Features
echo ""
...
curl -s -u "${JIRA_USER_EMAIL}:${JIRA_API_TOKEN}" \
  ...
  }" > "$OUTPUT_DIR/cntin_fy26_feat_${DATE_STR}.json"

FEAT_COUNT=$(cat "$OUTPUT_DIR/cntin_fy26_feat_${DATE_STR}.json" | python3 -c "import json,sys; d=json.load(sys.stdin); print(len(d.get('issues', [])))")
echo "  ✓ 找到 $FEAT_COUNT 个 Features"

if [ "$FEAT_COUNT" -eq "0" ]; then
    echo "⚠️ 没有找到关联的 Features"
    # 继续生成只有 Initiatives 的报告
fi

# 获取 Feature keys
FEAT_KEYS=$(cat "$OUTPUT_DIR/cntin_fy26_feat_${DATE_STR}.json" | python3 -c "
import json,sys
data=json.load(sys.stdin)
issues=data.get('issues',[])
keys=[i['key'] for i in issues]
...
if [ "$FEAT_KEYS" != "NONE" ]; then
    curl -s -u "${JIRA_USER_EMAIL}:${JIRA_API_TOKEN}" \
      ...
      -d "{
        ...
        \"maxResults\": 1000,
        \"fields\": [\"key\", \"summary\", \"description\", \"status\", \"assignee\", \"created\", \"updated\", \"parent\", \"customfield_10014\"]
      }" > "$OUTPUT_DIR/cntin_fy26_epic_${DATE_STR}.json"
    
    EPIC_COUNT=$(cat "$OUTPUT_DIR/cntin_fy26_epic_${DATE_STR}.json" | python3 -c "import json,sys; d=json.load(sys.stdin); print(len(d.get('issues', [])))")
    echo "  ✓ 找到 $EPIC_COUNT 个 Epics"
else
    echo "  ⚠️ 跳过 Epics（没有 Features）"
    echo '{"issues": []}' > "$OUTPUT_DIR/cntin_fy26_epic_${DATE_STR}.json"
    EPIC_COUNT=0
fi
...
# 加载数据
with open(f"{OUTPUT_DIR}/cntin_fy26_init_{DATE_STR}.json", "r") as f:
    init_data = json.load(f)
with open(f"{OUTPUT_DIR}/cntin_fy26_feat_{DATE_STR}.json", "r") as f:
    feat_data = json.load(f)
with open(f"{OUTPUT_DIR}/cntin_fy26_epic_{DATE_STR}.json", "r") as f:
...
# 保存 HTML
html_path = f"{OUTPUT_DIR}/cntin_fy26_epic_report_{DATE_STR}.html"
...
# 同时生成 CSV 报告
csv_path = f"{OUTPUT_DIR}/cntin_fy26_epic_report_{DATE_STR}.csv"
with open(csv_path, "w", encoding="utf-8-sig") as f:
    f.write("Initiative Key,Initiative Summary,Feature Key,Feature Summary,Epic Key,Epic Summary,Epic Status,Epic Description\\n")
    for item in epic_hierarchy:
        desc = item['epic_desc'].replace('"', '""')  # Escape quotes
        f.write(f'"{item["initiative_key"]}","{item["initiative_summary"]}","{item["feature_key"]}","{item["feature_summary"]}","{item["epic_key"]}","{item["epic_summary"]}","{item["epic_status"]}","{desc}"\\n')
...
echo ""
echo "📁 生成的文件:"
ls -lh "$OUTPUT_DIR"/cntin_fy26_*"${DATE_STR}".*