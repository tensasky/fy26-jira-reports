...
JIRA_EMAIL = os.getenv("JIRA_EMAIL") or os.getenv("JIRA_USER_EMAIL", "rcheng2@lululemon.com")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"   ⚠️ SSL 错误，等待 {wait_time}s 后重试 {attempt + 1}/{max_retries}...")
                ...
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt
                    print(f"   ⚠️ 请求错误，等待 {wait_time}s 后重试 {attempt + 1}/{max_retries}...")
                    time.sleep(wait_time)
                else:
                    raise
